# Manuel ultime de musculation

*Ouvrage de musculation — accessoires (secondaire)*

## L’auteur et le livre

Manuel orienté hypertrophie / musculation classique. Utile comme **catalogue d’accessoires free-weight** (dos, épaules, tronc) — pas comme méthodo CrossFit élite.

## Vision globale

Des muscles et tendons solides aident la gym et la barre. Mais le bodybuilding (splits, machines, volume d’isolation massif) n’est **pas** l’objectif : on prend des idées d’accessoires courts, compatibles box et créneau 90 min.

## Concepts essentiels

- Accessoires : row variations, work d’épaules, tronc, postérieur.
- Hypertrophie locale au service de la résilience (épaules, rachis), pas de l’esthétique.
- Volume d’accessoires = reste du budget temps après le travail principal.

## Ce qu’on retient pour ta prog

- 1–2 accessoires en fin de séance si le temps le permet.
- Free weights only (halteres, barre, kettlebells, bandes…) — **aucune machine guidée**.

## Ce qu’on n’applique pas

- Splits bodybuilding / « push pull legs » comme structure de semaine.
- Machines (leg press, smith, poulies machine…).
- Hypertrophie comme intention dominante d’un meso (sauf besoin tissus très ciblé).

## À retenir

- Secondaire et court.
- Au service de la gym et de la barre.
- Box only.
- Ne pilote jamais le cycle.

## Où ça se voit

Fin de séance ponctuelle ; jamais en « Fondements (corpus) » d’une semaine pilote.
