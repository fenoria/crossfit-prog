# Verkhoshansky — Special Strength Training

*Yuri Verkhoshansky — transfert & force spéciale*

## L’auteur et le livre

Manuel coach sur la **force spéciale** (SST) : comment organiser les moyens de force pour qu’ils se transforment en performance dans la discipline. Cohérent historiquement avec les blocs concentrés (proche de la logique Issurin).

## Vision globale

Devenir plus fort en salle ne garantit pas d’être meilleur en compétition. Il faut une **chaîne de transfert** : force générale → force orientée → expression sous le régime du sport (chez toi : mixed CrossFit — gym + barre + ergo sous fatigue).

Les mesos de **Transmutation** existent pour ça : convertir ce qui a été construit en Accumulation.

## Concepts essentiels

### Force spéciale (SST)

Moyens choisis pour coller au régime moteur de la discipline (durée d’effort, enchaînements, contraintes techniques), pas seulement au 1RM.

### Organisation concentrée dans l’année

Les blocs de force ne sont pas saupoudrés au hasard : concentration, puis conversion — aligné Issurin.

### Boîte à outils (à sélectionner)

Sauts, circuits, coupling, travail force-aérobie… On **sélectionne** selon le meso ; on ne colle pas un programme aviron ou sprint tel quel, ni un Westside « brut ».

## Ce qu’on retient pour ta prog

- Guide le passage Accumulation → Transmutation mixed → Affûtage.
- Dans le mixed : gym + barre + conditioning **sous fatigue**, pas des qualités isolées éternellement.
- Justification des WODs / densités en phase de conversion.

## Ce qu’on n’applique pas

- Programmes sport-par-sport copiés.
- Conjugate Westside sans adaptation box / 90 min.
- Force pour la force sans date de transfert.

## À retenir

- Fort ≠ compétitif tant que le transfert n’est pas programmé.
- Transmutation = phase dédiée, pas un accident.
- Sélectionner les moyens SST, ne pas tout faire.
- Complète Zatsiorsky (principes) + Issurin (blocs).

## Où ça se voit

Meso Transmutation mixed du Macrocycle 1 : conversion vers le stimulus compétition.
