...
Cette page résume **comment on programme** — pour qu’un coach puisse lire la logique d’ensemble sans devoir ouvrir chaque semaine.
...
5. **Santé > ego** — charges basées sur les benchmarks **actuels** (pas d’anciens records) ; variantes power / techniques contrôlées si confiance barre ou tissus le demandent ; douleur signalée → on adapte.
...
| **Accumulation force** | Force max / structure | Après (ou avec) base gym |
...
Dans un meso concentré, la logique hebdo suit souvent :

1. **Volume adaptatif** → 2. **Surcharge** → 3. **Pic** (densité / intensité) → 4. **Deload** (~−30 % volume)

Cadre pratique :

- Lun–Ven : créneau **~90 min** (échauffement inclus)
- Mercredi : team WOD possible (compte dans la dose)
- Samedi : **optionnel** — volume / team / Z2 selon dispo
- Dimanche : off, ou Z2 si créneau

### Domaines — comment on les traite

| Domaine | Approche |
|---------|----------|
| **Gym** (priorité actuelle) | Skill + volume (EMOM, densité) + renfo lesté. Une qualité dominante par jour en meso gym. Pas de junk-volume. |
| **Force** | Patterns squat / hinge / press / pull. Accumulation contrôlée (pas de max agressifs). Squats mesurés ; confiance barre > ego. |
| **Haltéro** | Technique + pulls + **power** en priorité. Squat snatch / clean lourds : rares, submax, stop si peur ou douleur. |
| **Conditioning** | **Maintien** (déjà solide). Z2 fréquent. HI placé surtout en conversion / pic — pas d’accumulation HI gratuite. |
| **Mental** | Actif surtout en pré-comp (routines, stress) — pas au détriment du travail physique en accumulation. |

### Dose & progression

- Intensité en **RPE** et **% sur charges actuelles** (benchmarks post-calibration).
- Progression : d’abord **qualité / volume de reps**, ensuite charge.
...
- Douleur signalée → protocole adapté (pas de score quotidien « adducteur » demandé).
...
Douze ouvrages — **[fiches de lecture](/livres/)** :
...
| Dose / fatigue | [Israetel et al. — *Scientific Principles of Strength Training*](/livres/israetel-scientific) | **Pilote** : dose utile vs trop + deloads |
| Force | [Zatsiorsky & Kraemer — *Science and Practice of Strength Training*](/livres/zatsiorsky-spst) | Principes force |
| Transfert | [Verkhoshansky — *Special Strength Training*](/livres/verkhoshansky-sst) | Force → performance spécifique CF |
...
Quand les auteurs divergent, un **pilote** tranche ; les autres soutiennent :

- **Issurin** pilote la séquence des mesos ; **Bompa** cadre le macro (2–3 pics / an).
- **Israetel** pilote la dose (zone utile + deload).
- **Gym = Low** en meso dédié · **Oly = Everett** (power avant squat lourd) · **Force = Zatsiorsky + transfert Verkhoshansky**.
- **Conditioning = Laursen & Buchheit** (cibles / formats) + Bible PP en socle ; maintien, pas sur-stimulation.
...
1. **Saison** — plan annuel  
2. **Macrocycle** — stage Acc → Trans → Real  
3. **Meso** — intention dominante + critères de passage  
4. **Semaine** — séances du jour (cibles chiffrées)

## En cours

- [Saison 2026](saison-2026/index.md) → Macrocycle 1 — Build → Meso Benchmarks → [S01](saison-2026/macrocycle-1-build/meso-benchmarks/S01-2026-08-03.md)
