# Meso — Expression (Fire)

<!-- Coach : code REAL -->

**Macrocycle** : 1 — Build → Fire · **Saison** : 2026  
**Durée prévue** : 1 sem. (7–13 septembre) · **Statut** : rédigé (S06) · à venir (exécution)
**Événement** : Fire Contest (Nantes) — rôle B · ~12–13 septembre

## Pourquoi

Ancrer une phase d’expression sur le pic B (Bompa multi-pic). Fraîcheur, pas un taper A ultra-long : Fire reste secondaire face à S7mbioz en décembre.

## Intention dominante

**Type de bloc** : Réalisation (pic B)  
**Stimulus** : volume bas, intensité utile, spécificité légère team mixte à 2 ; weekend compétition.  
**Ce que ce n’est pas** : accumulation, deload « off » total, mock gratuit en plus de la compet.

## Apport

- Court : arriver frais et compétitif à Nantes  
- Moyen : notes peaking (ce qui a marché) pour S7mbioz  
- Long : second pic annuel sans casser la progression gym/force

## Continuité

- **Précède** : Accumulation gym — deload / fraîcheur OK  
- **Suit** : Transition mini — compétition faite (ou skip documenté)

## Focus par semaine

| Semaine | Focus | Contenu type |
|---------|-------|--------------|
| S06 (7–13 sept.) | Expression Fire | Lun–jeu : volume bas, ouverture légère ; ven : repos ou Z2 très court ; sam–dim : compétition |

## Maintien (hors focus)

Tout hors compétition reste léger : pas de HI ajouté, pas de max force, gym en ouverture seulement si utile.

## Métriques de succès (sortie)

- [ ] Weekend Fire Contest fait ou skip documenté  
- [ ] Notes peaking (charges, RPE, format team, fatigue) dans le fichier semaine  
- [ ] Pas de surcharge inutiles la semaine du B

## Semaines

- [S06 — 7–13 septembre 2026](S06-2026-09-07.md) — expression Fire Contest

## Notes meso

_(résultat Fire ; arbitrage charges pour Macro 2)_
